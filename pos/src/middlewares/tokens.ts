...
function verifyTokenAndCheckOwner(req: IReq, res: IRes, next: e.NextFunction): void {
  try {
    const secretKey = process.env.JWT_SECRET as string;
...
function verifyTokenAndCheckAdmin(req: IReq, res: IRes, next: e.NextFunction): void {
  try {
    ...
    const decodedToken = jwt.verify(token, secretKey) as TokenPayload;
    const ACCEPTED_ROLES = ['Owner',"Manager", "Admin"]

    if (decodedToken.role in ACCEPTED_ROLES) {
      // User role is admin
      next();
    } else {
      // User role is not admin
